...
def group_mass_tab_vold(nmin=5, edge_min=0.9, mbins=mbins_def,
...
def group_mass_tab_old(nmin=5, edge_min=0.9, mbins=mbins_def,
                   out_file='group_mass.tex'):
    """Tabulate group mass statistics."""
    ...
    # Numbers and mean log-masses of TNG300 groups
    ...
    ngrp_mock, ngal_mock, zmean_mock, Mmean_mock = group_stats(g3cmockfof)
    nmbin = len(mbins) - 1
...
\hline''', file=f)
    for i in range(nmbin):
        print(r'${\cal M}' + str(i+1) + r'$ & ' +
              ...
              ngrp_gama[i] + ' & ' + Mmean_gama[i] +
              ' & ' + zmean_gama[i] + ' & & ' +
              ngrp_mock[i] + ' & ' + Mmean_mock[i] +
              ' & ' + zmean_mock[i] + r'\\', file=f)
    print(r'''
\hline
\end{tabular}
\end{table}
...
\multicolumn{3}{c}{FoF Mocks} \\
\cline{3-5} \cline{7-9} \cline{11-13} \\[-2ex]
& $\lg {\cal M}_{h, {\rm limits}}$ & $N$ & $\overline{\lg {\cal M}}$ &
$\overline z$ & &
...
\hline''', file=f)
    for i in range(nmbin):
        print(r'${\cal M}' + str(i+1) + r'$ & ' +
              ...
              ngrp_gama[i] + ' & ' + Mmean_gama[i] +
              ' & ' + zmean_gama[i] + ' & & ' +
              ngrp_hmock[i] + ' & ' + Mmean_hmock[i] +
              ' & ' + zmean_hmock[i] + ' & & ' +
              ngrp_fmock[i] + ' & ' + Mmean_fmock[i] +
...
For each simulation, we give the number of groups, mean log-mass,
and snapshot redshift.
\label{tab:sim_group_mass_def}}
\begin{tabular}{ccccccccccccccccc}
\hline
& & \multicolumn{3}{c}{\lgal} & &
\multicolumn{3}{c}{EAGLE} & & \multicolumn{3}{c}{\tng} \\
\cline{3-5} \cline{7-9} \cline{11-13} \\[-2ex]
& $\lg {\cal M}_{h, {\rm limits}}$ & $N$ & $\overline{\lg {\cal M}}$ & z &
& $N$ & $\overline{\lg {\cal M}}$ & z & & $N$ & $\overline{\lg {\cal M}}$ & z \\
\hline''', file=f)
    for i in range(nmbin):
        lgal_lf = lgal_dict[f'M{i}Lgalcen']
        eagle_lf = eagle_dict[f'M{i}eaglecen']
        tng_lf = tng_dict[f'M{i}tng']
        print(r'${\cal M}' + str(i+1) + r'$ & ' +
              '[{:4.1f}, {:4.1f}] & '.format(mbins[i], mbins[i+1]) +
              rf"{lgal_n[i]} & {lgal_lgm[i]} & {lgal_z[i]} & & " +
#              rf"{lgal_lf.sel_dict['Nhalo']} & {lgal_lf.sel_dict['Mav']:5.2f} & {lgal_z[i]} & & " + 
              rf"{eagle_lf.sel_dict['Nhalo']} & {eagle_lf.sel_dict['Mav']:5.2f} & {eagle_z[i]} & & " + 
              rf"{tng_lf.sel_dict['Nhalo']} & {tng_lf.sel_dict['Mav']:5.2f} & {tng_z[i]} \\", file=f)
    print(r'''
\hline
\end{tabular}
\end{table*}
...
def smf_field(kref=0.0, masscomp=1, colname='logmstar', clrname='gi_colour',
...
def gama_mock_plot(gfile='lfr_k00.pkl', mfile='lfr_mock.pkl',
                   lf_lims=(-15, -23.5, 1e-6, 0.1), nmin=5, schecdbl=True,
...
#    axin.set_ylabel(r'$M^*$')
    ...
    phig = gdict['all']
    phim = mdict['all']
    for phi, label, clr in zip((phig, phim), ('GAMA', 'Mock'), 'br'):
        phi.fn_fit(fn, Mmin=Mmin_fit, Mmax=Mmax_fit)
...
def field_smf_plot(#gfiles=('smf_field_incomp.pkl', ),
                   gfiles=('smf_field_comp.pkl', 'smf_field_incomp.pkl'),
                   lf_lims=(7, 12.5, 1e-5, 1), nmin=5, fn=SchecMag(),
                   p0=(-1, -21, -2), Mmin_fit=8, Mmax_fit=12.5,
                   sigma=[1, 2], lc_limits=5, lc_step=32,
                   plot_file='/Users/loveday/Documents/tex/papers/gama/groupLF/smf_field_comp.pdf',
                   plot_size=(6, 6), alpha=0.8):
...
#    clr = 'b'
#    phi.fn_fit(fn, Mmin=Mmin_fit, Mmax=Mmax_fit)
        # phi.plot(ax=ax, nmin=nmin, label='This work', alpha=alpha)
    ...
    phi = bdat[:, 2]/1000/h**3
    phi_err = bdat[:, 3]/1000/h**3
    plt.errorbar(logM, phi, phi_err, fmt='s', label='Baldry+2012', alpha=alpha)

    # Wight+2017
    wright = (10.78 + 2*lgh, -0.62, -1.50, 2.93e-3/h**3, 0.63e-3/h**3)
    phi = lf.LF(None, None, logM)
    phi.ref_fn(phi.Schechter_dbl_mass, wright)
    phi.plot(ax=ax, fmt='-', label='Wright+2017', alpha=alpha)
    plt.axis(lf_lims)

    # L-Galaxies SMF
    infile = lf_data + 'L-Galaxies/MR_MRII_smf_z0.20.txt'
    data = np.loadtxt(infile)
    mass, phi, phi_err = data[:, 0], data[:, 1], data[:, 2]
    ax.errorbar(mass, phi, phi_err, label='L-Galaxies', alpha=alpha)
    ax.legend()

    # EAGLE SMF
    infile = lf_data + '/EAGLE/smf.dat'
    (mass, phi, phi_err) = np.loadtxt(infile)
    ax.errorbar(mass, phi, phi_err, label='EAGLE', alpha=alpha)
    ax.legend()
#    print(mass, phi, phi_err)

    # TNG LF
    phi = pickle.load(open('tng_smf.pkl', 'rb'))
    phi.plot(ax=ax, nmin=nmin, fmt='-', label='TNG', alpha=alpha)
...
def smf_zslice(colname='logmstar', bins=np.linspace(7, 12, 21), masscomp=True,
               Vmax='Vmax_dec'):
...
def smf_z(colname='logmstar', bins=np.linspace(7, 12, 21),
          masscomp=True, outfile='smf_comp_z.pkl'):
...
#                gauss_lims=(-7, -2.7, 10.2, 11.3),
                yang_pref='csmf_', tng_file='tng_csmf.pkl',
...
#            fig.text(0.52, 0.0, r'$\lg \phi^*$', ha='center', va='center')
            fig.text(0.06, 0.45, r'$M_c$', ha='center', va='center',
                     rotation='vertical')
        else:
            fig.text(0.52, 0.0, r'$\alpha$', ha='center', va='center')
            fig.text(0.06, 0.45, mstard, ha='center', va='center',
                     rotation='vertical')
        lines = []
...
def csmf_plots3(
        # panels=(('LGAL', 'all'), ('GAMA', 'all'),
        #                 ('GAMA', 'blue'), ('GAMA', 'red'),
        #                 ('EAGLE', 'all'), ('TNG', 'all'),
        #                 ('TNG', 'blue'), ('TNG', 'red')),
        panels=(('GAMA', 'all'), ('GAMA', 'blue'), ('GAMA', 'red'),
                ('GAMA', 'nlo'), ('GAMA', 'nhi'),
                ('TNG', 'all'), ('TNG', 'blue'), ('TNG', 'red'),
                ('LGAL', 'all'), ('EAGLE', 'all')),
        gama_file='smf_comp_vmax.pkl', tng_file='tng_csmf.pkl',
        ...
        Mmin_fit=8.0, Mmax_fit=12.5,
        nmin=2, lf_lims=(8.2, 12.2, 5e-8, 0.02),
        schec_lims=(-1.8, 0.1, 10.3, 11.3),
        gauss_lims=(0.12, 0.38, 10.25, 11.9),
        yang_pref='csmf_',
        eagle_file='eagle_csmf.pkl', Lgal_file='smf_Lgal.pkl',
        cen_table='csmf_gauss.tex', sat_table='csmf_schec.tex',
        plot_files=('csmf.pdf', 'csmf_gauss.pdf', 'csmf_schec.pdf'),
        xlabel=ms_label, ylabel=smf_label,
        sigma=[1, ], lc_limits=5, lc_step=32,
        plot_size=(6, 7.5), plot_shape=(5, 2),  # lot_shape=(2, 4),
        red_frac_file='red_frac.pkl', main_plot_size=(5, 10), # main_plot_size=(12, 5.2),
        contour_plot_size=(5, 10)):
        # contour_plot_size=(12, 5.2)):
    """Plot and tabulate galaxy CSMFs by colour and Sersic index,
    separately for central/satellite."""
    clf_plots3(panels=panels, gama_file=gama_file, mock_file=mock_file,
               schec_fn=schec_fn, gauss_fn=gauss_fn, schecdbl=schecdbl,
               schec_p0=schec_p0, gauss_p0=gauss_p0,
               Mmin_fit=Mmin_fit, Mmax_fit=Mmax_fit,
               nmin=nmin, lf_lims=lf_lims,
...
def clf_plots3(panels=(('GAMA', 'all'), ('GAMA', 'blue'), ('GAMA', 'red'),
                       ('GAMA', 'nlo'), ('GAMA', 'nhi')),
               gama_file='lf_vmax.pkl', tng_file='tng_clf.pkl', mock_file=None,
               schec_fn=SchecMagSq(), gauss_fn=LogNormal(), schecdbl=False,
               schec_p0=[[-20.2, -22, -19], [-1.2, -2, 1], [-2, -8, 0]],
               gauss_p0=[[-21.5, -23, -20], [0.5, 0.1, 0.8], [-4, -7, -2]],
               Mmin_fit=-24, Mmax_fit=-16,
...
#    f1 = SchecMag('schec1')
    ...
    fig1.subplots_adjust(left=sa_left, bottom=sa_bot, right=1.0, top=0.95,
                         hspace=0.0, wspace=0.0)
    fig1.text(0.58, 0.0, xlabel, ha='center', va='center')
    fig1.text(0.0, 0.5, ylabel, ha='center', va='center', rotation='vertical')
    plt.semilogy(basey=10, nonposy='clip')
    plt.axis(lf_lims)
    # cbar_ax = fig1.add_axes([0.92, 0.08, 0.01, 0.82])
    cbar_ax = fig1.add_axes([0.18, 0.98, 0.82, 0.02])
    ...
    fig2.subplots_adjust(left=sa_left, bottom=sa_bot, top=1.0,
                         hspace=0.0, wspace=0.0)
    fig2.text(0.52, 0.0, r'$\sigma_c$', ha='center', va='center')
    ...
    fig3.subplots_adjust(left=sa_left, bottom=sa_bot, top=1.0,
                         hspace=0.0, wspace=0.0)
    fig3.text(0.52, 0.0, r'$\alpha$', ha='center', va='center')
...
def clf_mock_comp2(mockfof_file='lf_mockfof.pkl',
                   ...
                   nmin=8, lf_lims=(-15.5, -23, 2e-7, 0.005),
                   schec_lims=(-1.9, 1.1, -21.6, -19.1),
                   schec_fn=SchecMagSq(), gauss_fn=LogNormal(),
                   Mmin_fit=-23, Mmax_fit=-17,
                   schec_p0=[[-20.2, -22, -19], [-1.2, -2, 1], [-2, -8, 0]],
                   gauss_p0=[[-21.5, -23, -20], [0.5, 0.1, 0.8], [-4, -7, -2]],
                   sigma=[1, 2], lc_limits=5, lc_step=32,
                   plot_file='clf_mock_comp.pdf', alpha=0.5,
                   xlabel=mag_label, ylabel=lf_label):
...
def csmf_ev_plots(intemp='smf_comp_vmax_z{}_{}.pkl', gal_file='smf_comp_z.pkl',
                  fn=SchecMass(),
                  p0=(-1.5, 10.5, 1), nmin=3, lf_lims=[8.1, 12.0, 1e-6, 3e-3],
                  Mmin_fit=8.8, Mmax_fit=12, schec_lims=(9.6, 11.0, -4, -2.1),
    ...
    clf_ev_plots(intemp=intemp, gal_file=gal_file, fn=fn,
                 ...
                 plot_size=plot_size,
                 xlabel=xlabel, ylabel=ylabel, alpha=alpha, plot_ratio=plot_ratio)


def clf_ev_plots(intemp='lf_vmax_z{}_{}.pkl', gal_file='lfr_z.pkl',
                 mocktemp='lf_mock_z{}_{}.pkl', fn=SchecMag(),
                 mock_gal_file='lfr_z_mock.pkl',
                 p0=(-1, -20, 1), nmin=3, lf_lims=[-15.7, -23.6, 1e-6, 2e-3],
                 Mmin_fit=-24, Mmax_fit=-17, schec_lims=(-19.4, -22, -3.6, -2.3),
                 plot_file='clf_ev.pdf', plot_schec_file='clf_ev_schec.pdf',
                 plot_size=(12, 6.5), censat=False,
                 mkey=(3, 4, 56), xlabel=mag_label, ylabel=lf_label,
                 zlabel=[r'$0.0 < z < 0.1$', r'$0.1 < z < 0.2$',
                         r'$0.2 < z < 0.3$'],
                 sigma=[1, ], lc_limits=5, lc_step=21, alpha=0.5, plot_ratio=0):
...
#                if censat:
#                    for cs in ('cen', 'sat'):
#                        phi = lf_dict_list[iz][key + cs + lbl]
#                        if cs == 'cen':
#                            phi.plot(ax=ax1, nmin=nmin, clr=colors[iz],
#                                     show_fit=False, label=zlabel[iz], alpha=0.5)
#                        else:
#                            phi.plot(ax=ax1, nmin=nmin, ls='--', clr=colors[iz],
#                                     mfc='w', alpha=0.5)
#                else:
                phi = lf_dict_list[iz][key + '_all' + '_' +lbl]
#                key = 'M{}z{}{}'.format(mkey[im], iz, lbl)
                gkey = 'z{}_{}'.format(iz, lbl)
#                if mock_file:
#                    phi = lf_dict[key + 'mock']
#                    phi.plot(ax=ax, nmin=nmin, ls=':', mfc='w', show_fit=False)
#                if mocktemp and it == 0:
#                    phi = mock_dict_list[iz][key + 'mock']
#                    gphi = mock_gal_dict[gkey + 'mock']
#                else:
#                phi = lf_dict_list[iz][key]
                gphi = gal_dict[gkey]
...
#                phi.plot(ax=ax1, nmin=nmin, clr=colors[iz], show_fit=False,
#                         label=zlabel[iz])
                if plot_ratio:
                    ...
                    ax1.errorbar(phi.Mbin[comp], phir, phir_err, fmt='o',
...
#                            colors=colors[iz])
            if not(plot_ratio) and im == mbins[0] and it == 0:
                ax1.legend(loc=3)
    plt.draw()
    plt.figure(1)
    plt.savefig(plot_dir + plot_file, bbox_inches='tight')
...
#            Vmax='Vmax_raw', outfile='csmf_sim_nmin1_vmax.pkl')
    clf_sim(nmin=5, gal_file='smf_gal.fits', grp_file='smf_groupfits',
            colname='logmstar', bins=np.linspace(8, 12, 17),
            Vmax='Vmax_raw', outfile='csmf_sim_nmin5_vmax.pkl')
...
def tng_smf(sim='TNG300-1', snap=87, mlim=18,
            mhbins=mbins_sim,
    ...
    smcorr = 1.4  # stellar mass resolutioncorrection factor (Pillepich+18 A1)
...
#    lgMs = 10 + np.log10(h*subhalos['SubhaloMassInRadType'][sel, 4])
    lgMs = 10 + np.log10(smcorr*subhalos['SubhaloMassInRadType'][sel, 4])
    ...
    plt.semilogy(basey=10)
    plt.xlabel(r'$M_* [M_\odot/h]$')
    plt.ylabel(r'$\phi(M_*)$')
    plt.title('Field SMF')
    plt.show()

    # Field LF
    hist, edges = np.histogram(Mr, bins=mrbins)
    phi = hist/boxvol/msbinsize
    phierr = hist**0.5/boxvol/msbinsize
    ...
    ngal = len(host_mass[(host_mass >= mhbins[0]) * (host_mass < mhbins[-1])])
...
def Lgal_plot(what='lf'):
    ...
    plt.ylabel('Phi')
    plt.semilogy()
    cen_smf_dir = read_Lgal(f'/Users/loveday/Documents/Research/LFdata/L-Galaxies/MR_MRII_halo_{what}_centrals.txt')
    sat_smf_dir = read_Lgal(f'/Users/loveday/Documents/Research/LFdata/L-Galaxies/MR_MRII_halo_{what}_satellites.txt')
    smf_dir = {}

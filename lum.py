...
def lfFaint(inFile='kcorrz.fits', absStep=0.25, bands=list(range(5)), colours='cbr',
...
def lfSDSSFaint(inFile='kcorrz.fits', absStep=0.25, bands=list(range(5)),
...
##         idx *= (bpt == bpt_type)
    ...
    r = cosmo.dm(z)
    z1 = [zdm(appMin - absMag[i], kcoeff[i], zRange) for i in range(ngal)]
    r1 = cosmo.dm(z1)
    z2 = [zdm(appMax - absMag[i], kcoeff[i], zRange) for i in range(ngal)]
    r2 = cosmo.dm(z2)
    vol = r*r*r - r1*r1*r1
...
##     print 'anneal:', result
    ...
    if errorEst:
        for i in range(npar):
            imarg = [j for j in range(npar) if j != i]
    ...
    if 'P' in schec:        
        ...
        z1 = [zdm(par['appMin'] - Mevol[i], samp.kcoeff[i],
                               samp.zRange, ev) for i in range(samp.ngal)]
        z2 = [zdm(par['appMax'] - Mevol[i], samp.kcoeff[i],
                               samp.zRange, ev) for i in range(samp.ngal)]
...
##     nstep = 21
    ...
    for i in range(npar-1):
        ...
        for j in range(i+1, npar):
            ...
            print(par['iband'], xpar, ypar, xopt, yopt, maxLike, nstep, xmin, xmax, ymin, ymax, file=f)
            ...
            imarg = [k for k in range(npar) if k != i and k != j]
            x0init = []
            parOrd = []
            margstr = ''
            if npar > 2:
...
##                                 pdb.set_trace()
    ...
    if 'P' in schec:        
        ...
        print(par['iband'], 'Q', 'P', Q, P, maxLike, nstep, \
              Qmin, Qmax, Pmin, Pmax, file=f)
...
##                             print c, x0, [x, y], L
        ...
        print(par['iband'], xpar, ypar, xopt, yopt, maxLike, nstep, \
              xmin, xmax, ymin, ymax, file=f)
...
def swmlNorm(samp, phi):
    ...
    usegal = [gal[i] for i in range(ngal) if phi.use[gal[i].phiBin]]
    nuse = len(usegal)
    weight = np.array([usegal[i].weight for i in range(nuse)])
    wmean = weight.mean()
    selg = np.array([phi.sel(usegal[i].z) for i in range(nuse)])
...
class BinnedPhi(object):
    ...
    def save(self, f):
        ...
        print([self.zRange, self.zmean, self.ntot, self.counts, self.kmean], file=f)
        print(self.nbin, self.absMin, self.absMax, self.lnLikeRatio, self.nu, self.prob, file=f)
        for i in range(self.nbin):
            print(self.ngal[i], self.Mav[i], \
                  self.Vmax[i], self.VmaxErr[i], \
                  self.swml[i], self.swmlErr[i], self.sty[i], file=f)
...
class lfData(object):
    ...
    def write(self, outFile):
        ...
        print(self.schec, file=f)
        print(self.lumdens, file=f)
...
def jackErrs(obsFile, jackList, outFile):
    ...
    for key in ('alpha', 'Mstar', 'Q', 'P', 'beta', 'Mt', 'phistar'):
        if key in lf.schec:
            err = np.sqrt(
                jfac*np.var([lfList[i].schec[key][0] for i in estr]))
            lf.schec[key] = (lf.schec[key][0], err)
    
    key = 'counts'
    if key in lf.schec:
        err = np.sqrt(
            jfac*np.var([lfList[i].schec[key] for i in estr]))
        lf.schec[key] = (lf.schec[key], err)
    
    # Luminosity density
    err = np.sqrt(jfac*np.var([lfList[i].lumdens['ld0'][0] for i in estr]))
    lf.lumdens['ld0'] = (lf.lumdens['ld0'][0], err)
    err = np.sqrt(jfac*np.var([lfList[i].lumdens['ld'][0] for i in estr], 0)).tolist()
        ...
        for ibin in range(phi.nbin):
            ...
            if ngmin > 0:
                phi.VmaxErr[ibin] = np.sqrt(jfac*np.var(
                    [lfList[i].phiList[iz].Vmax[ibin] for i in estr]))
                phi.swmlErr[ibin] = np.sqrt(jfac*np.var(
                    [lfList[i].phiList[iz].swml[ibin] for i in estr]))
            else:
...
def simAv(fileList, outFile):
    ...
    for key in ('alpha', 'Mstar', 'Q', 'P', 'beta', 'Mt', 'phistar'):
        if key in lf.schec:
            val = np.mean([lfList[i].schec[key][0] for i in estr])
            err = np.std([lfList[i].schec[key][0] for i in estr])
            lf.schec[key] = (val, err)
    
    # Luminosity density
    val = np.mean([lfList[i].lumdens['ld0'][0] for i in estr])
    err = np.std([lfList[i].lumdens['ld0'][0] for i in estr])
    lf.lumdens['ld0'] = (val, err)

    val = np.mean([lfList[i].lumdens['ld'][0] for i in estr], 0).tolist()
    err = np.std([lfList[i].lumdens['ld'][0] for i in estr], 0).tolist()
    ...
    for iz in range(lf.par['nz']):
        ...
        phi.lnLikeRatio = np.mean(
            [lfList[i].phiList[iz].lnLikeRatio for i in estr])
        phi.nu = np.mean([lfList[i].phiList[iz].nu for i in estr])
        phi.prob = np.mean([lfList[i].phiList[iz].prob for i in estr])
        ...
        for ibin in range(phi.nbin):
            ...
            if ngmin > 0:
                phi.Mav[ibin] = sum(
                    [lfList[i].phiList[iz].ngal[ibin]*
                        lfList[i].phiList[iz].Mav[ibin] for i in estr])/ngsum
                phi.ngal[ibin] = sum(
                    [lfList[i].phiList[iz].ngal[ibin] for i in estr])
                ...
                phi.sty[ibin] = np.mean(
                    [lfList[i].phiList[iz].sty[ibin] for i in estr])
            else:
                phi.ngal[ibin] = phi.Mav[ibin] = phi.Vmax[ibin] = phi.VmaxErr[ibin] = phi.swml[ibin] =  phi.swmlErr[ibin] = phi.sty[ibin] = 0
        lf.phiList[iz] = phi
...
def tabulate(fileList = ('lf_faint_?c.dat', 'lf_faint_?b.dat', 'lf_faint_?r.dat'),
    ...
    for inFile in fileList:
        ...
        for iz in range(lf.par['nz']):
            phi = lf.phiList[iz]
            print(band, colour, phi.zRange, file=f)
            print('ngal    M     Vmax   VmaxErr     swml  swmlErr     sty', file=f)
            for ibin in range(phi.nbin):
...
def lsqEvFit(fileList = ('lf_ev8_??.dat'), outFile='lsqEv8Fit.dat'):
    ...
    for inFile in fileList:
        ...
        print(inFile, Q, QErr, P, PErr, file=fout)
    fout.close()
...
def saundResid(xxx_todo_changeme2, M, phi, phiErr):
    """Return chi^2 residual between binned phi estimate and Saunders fit."""
    (alpha, Mstar, sigma, lpstar) = xxx_todo_changeme2
    fc = 0
    phistar = 10**lpstar
    for ibin in range(len(M)):
        if phiErr[ibin] > 0:
...
##         ax.scatter(mag[idx], col[idx], 0.01)
    ...
    for i in range(len(ra)):
        print(cataid[i], ra[i], dec[i], file=file)
    file.close()
...
##     M = absMag[idx,:]
    ...
    r = np.array([float(len(np.where((Mlo1[i] <= M) * (M <= M[i]) *
                                  (Zlo1[i] <= Z) * (Z <= Z[i]))[0])) for i in galit])
    ...
    for mfs in mfarray:
        ...
        n = np.array([float(len(np.where((Mlo1[idx[i]] <= Mt) *
                                            (Mt <= Mhi2[i]) *
                                            (Zlo2[i] <= Zt) *
                                            (Zt <= Zt[i]))[0])) for i in galit])
